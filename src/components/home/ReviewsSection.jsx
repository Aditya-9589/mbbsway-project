import React, { useState, useEffect } from "react";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation } from "swiper/modules";

import "swiper/css";
import "swiper/css/navigation";
import "../../styles/ReviewsSection.css";

const BASE_URL = "https://admin.mbbsway.in/storage/app/";

const tabsConfig = {
    Google: {
        alt: "Google",
        icon: "https://mbbsway.in/static/media/homereviewvectorgoogleicon.b0eee2254e4f9d955e37.png",
    },
    Facebook: {
        alt: "Facebook",
        icon: "data:image/png;base64,",
    },
    Justdial: {
        alt: "justdial",
        icon: "https://mbbsway.in/static/media/homereviewinstaicon.0c6bafbdd175dfa21f04.png",
    }
}

const ReviewsSection = ({ reviewsData }) => {

    // ✅ 1. Hooks FIRST (always run)
    const [activeTab, setActiveTab] = useState("");

    // ✅ 2. Safe fallback (NO destructuring yet)
    const safeData = reviewsData || {};

    // ✅ 3. Safe destructuring
    const { title, sub_title, ...platformReviews } = safeData;

    // ✅ 4. Safe platforms array
    const platforms = Object.keys(platformReviews || {});

    // ✅ 5. Initialize state AFTER data arrives
    useEffect(() => {
        if (platforms.length > 0 && !activeTab) {
            setActiveTab(platforms[0]);
        }
    }, [platforms, activeTab]);

    // ✅ 6. Guard AFTER hooks
    if (!reviewsData) return null;

    const activeReviews = platformReviews[activeTab] || [];


    return (
        <div className="reviews-section-wrapper">
            <div className="reviews-section-container">

                {/* TABS */}
                <div className="reviews-tabs-wrapper">
                    <div className="reviews-tabs">
                        {platforms.map((platform) => (
                            <button
                                key={platform}
                                type="button"
                                onClick={() => setActiveTab(platform)}
                                className={`reviews-tab-btn ${activeTab === platform ? "reviews-tab-btn--active" : ""}`}
                            >
                                <img
                                    src={tabsConfig[platform]?.icon}
                                    alt={platform}
                                />
                            </button>
                        ))}
                    </div>
                </div>

                <div className="reviews-main-wrapper">
                    <div className="reviews-main-inner">

                        {/* Heading */}
                        <div className="reviews-heading-wrapper mb-6">
                            <h2 className="reviews-heading">{title}</h2>
                        </div>

                        {/* Main Layout */}
                        <div className="reviews-layout">

                            {/* Left Panel */}
                            <div className="reviews-left-panel">
                                <p className="reviews-rating">{sub_title}</p>
                                <div className="reviews-stars">⭐⭐⭐⭐⭐</div>
                                <span className="reviews-basedon">
                                    Based on <b>{activeReviews.length} reviews</b>
                                </span>

                                <div className="reviews-platform-logo">
                                    <img
                                        src={tabsConfig[activeTab]?.icon}
                                        alt={activeTab}
                                    />
                                </div>
                            </div>

                            {/* Right Panel */}
                            <div className="reviews-right-panel">
                                <div className="reviews-swiper-wrapper">
                                    <Swiper
                                        modules={[Navigation]}
                                        navigation
                                        loop={true}
                                        spaceBetween={60}
                                        slidesPerView={3}
                                        centeredSlides={true}
                                        className="reviews-swiper"
                                        breakpoints={{
                                            0: {
                                                slidesPerView: 1,
                                                centeredSlides: true,
                                            },
                                            600: {
                                                slidesPerView: 3,
                                                centeredSlides: true,
                                            },
                                            900: {
                                                slidesPerView: 3,
                                                centeredSlides: true,
                                            }
                                        }}
                                    >
                                        {/* {reviews.map((r, index) => ( */}
                                        {[...activeReviews].map((r) => (
                                            <SwiperSlide key={r.id}>
                                                <div className="review-card-wrapper">
                                                    <div className="review-card">

                                                        <div className="review-card-header">
                                                            <div className="review-card-user">
                                                                <img
                                                                    src={`${BASE_URL}${r.image}`}
                                                                    alt={r.name}
                                                                    className="review-user-img"
                                                                />
                                                                <div>
                                                                    <h3 className="review-user-name">{r.name}</h3>
                                                                    <p className="review-user-city">{r.address}</p>
                                                                </div>
                                                            </div>

                                                            <img
                                                                src="https://cdn-icons-png.flaticon.com/128/300/300221.png"
                                                                className="review-google-icon"
                                                                alt=""
                                                            />
                                                        </div>

                                                        <p className="review-card-text">
                                                            {r.comment}
                                                        </p>

                                                    </div>
                                                </div>
                                            </SwiperSlide>
                                        ))}
                                    </Swiper>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>

            </div>
        </div>
    );
};

export default ReviewsSection;
